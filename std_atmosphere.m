...
% If it runs and is accurate, full credit.  If it runs but not accurate,
%half credit.
...
elseif altitude < 91000
    ...
    t = 186.8673;
    p = exp(2.156582e-06*altitude^3 -4.836957e-04*altitude^2-0.1425192...
    *altitude + 13.47530);
    rho = exp(-3.322622e-06*altitude^3 +9.111460e-04*altitude^2-0.1425192...
    *altitude + 5.944694);
...
elseif altitude < 100001
    ...
    p = exp(3.304895e-05*altitude^3 -0.009062730e-04*altitude^2+0.6516698...
    *altitude - 11.03037);
    rho = exp(2.873405e-05*altitude^3 - 0.008492037*altitude^2+0.6541179...
    *altitude -23.62010);
    mu = mu_0_earth*((t/earth_t0)^1.5)*((earth_t0+earth_ts)/(t+earth_ts));
